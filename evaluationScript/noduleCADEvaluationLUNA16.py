...
seriesuid_label = 'pid'
coordX_label = 'center_x'
coordY_label = 'center_y'
coordZ_label = 'center_z'
diameter_mm_label = 'diameter'
...
def collectNoduleAnnotations(annotations, seriesUIDs):
    ...
    for seriesuid in seriesUIDs:
        ...
        for annotation in annotations[1:]:
            ...
            if int(seriesuid) == int(nodule_seriesuid):
...
def collect(annotations_filename, valpath):
    annotations = csvTools.readCSV(annotations_filename)

    with open(valpath, "r") as f:
        testfilelist = f.read().splitlines()
    seriesUIDs = []
    for seriesUID in testfilelist:
...
def noduleCADEvaluation(annotations_filename,results_filename,valpath,outputDir):
    '''
    ...
    (allNodules, seriesUIDs) = collect(annotations_filename,valpath)
